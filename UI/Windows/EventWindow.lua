-- RPE_UI/Windows/EventWindow.lua
RPE_UI          = RPE_UI or {}
...
local EventWindow = {}
_G.RPE_UI.Windows.EventWindow = EventWindow
EventWindow.__index = EventWindow
...
local function exposeCoreWindow(self)
    ...
    _G.RPE.Core.Windows.EventWindow = self
end
...
function EventWindow:BuildUI()
    ...
    self.bottomBorder = HBorder:New("RPE_Event_BottomBorder", {
        parent        = self.root,
        stretch       = true,
        thickness     = 5,
        y             = 0,
    ...
    self.content = Panel:New("RPE_Event_Content", {
        parent  = self.root,
        autoSize = true,
    })
    ...
    self.footer = Panel:New("RPE_Event_Footer", {
        parent  = self.root,
        autoSize = false,
    })
    ...
    local units = _G.RPE_UI.Windows.EventUnitsSheet.New({ parent = self.content})
    self.pages["units"] = units
    units.sheet:Hide()

    local settings = _G.RPE_UI.Windows.EventSettingsSheet.New({ parent = self.content })
...
function EventWindow:ShowTab(key)
