...
The Setup Wizard Editor is a powerful tool for dungeon masters and dataset admins to create custom character creation sequences for players. Each page represents a step in character creation.

To access the setup wizard editor:
...
- **Type**: Page type (``SELECT_RACE``, ``SELECT_CLASS``, etc.). See "Page Types and Configuration" below for details on each type.
